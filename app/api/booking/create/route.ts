...
export async function POST(request: NextRequest) {
  try {
    ...
    const checkInValue = String(body?.checkIn ?? "").trim();
    const checkOutValue = String(body?.checkOut ?? "").trim();
    ...
    if (hotelId === null && destinationId === null) {
      return NextResponse.json(
        {
          message: "Please select a hotel or destination.",
        },
        { status: 400 }
      );
    }
        ...
        databaseHotel =
          await prisma.hotel.create({
            data: {
              ...
              name: String(selectedHotel.name),
              location: String(
